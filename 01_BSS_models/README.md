...
3. **Data prep.** Reads `effort_combined.xlsx`, `interview_combined.xlsx`, `wes_commercial_tally.xlsx`, `charter_trips.xlsx`, `sampler_shifts.xlsx` and `crabbing_holidays.xlsx` from `04_input_files/`, plus `ingress_egress.xlsx` (pooled). Every input is an `.xlsx` workbook with a single `data` sheet (converted from CSV on 2026-07-16), and **six of the nine are BUILT** from the per-season creel workbooks in `raw/` by `build_all_inputs.R`; do not hand-edit those. When `use_osp_boat_counts = TRUE`, both drivers also read `WBL_boat_counts.xlsx` (the OSP daily port count of returning vessels) as a second boat-effort stream. **The crabbing fraction `f` is built from the samplers' private-boat INTERVIEW CONTACTS** in `interview_combined.xlsx` (a per-stratum logit random walk, with a combo-trip share `c` on its own walk); the `boats_crabbing` / `boats_total` columns in `ingress_egress.xlsx` are a second observation stream that is present but currently BLANK. This line named those columns as f's source until 2026-09-12, which would send a reader to an empty column.
