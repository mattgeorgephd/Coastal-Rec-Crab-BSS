...
### v6.9 (2026-06-20), Single-cell hierarchical scale collapsed (B1.7)

-   **The single-section hierarchical scale is removed.** With `G = 1` and `S = 1` (this pooled model is always single-cell, RMD line 812), the effort and CPUE levels were written hierarchically as `mu_E[g,s] = mu_mu_E[g] + eps_mu_E[g,s] * sigma_mu_E` (and the CPUE counterpart). With one cell this places three parameters on one identified quantity: `sigma_mu_E` has no between-section variation to inform it, so its posterior is prior-dominated with a heavy right tail (in the boat all-gear fit, mean 1.27 against median 0.77, 95% CI to 5.6), and it forms a funnel with `eps_mu_E`. After v6.8 (B1.6) made `sigma_IE` proper, this was the boat's last divergence source: the 59 residual divergent transitions localized on `sigma_mu_E` (standardized divergent-vs-bulk separation 0.36, the largest by far; bulk median 0.77 dropping to 0.55 in the divergent draws), and those draws shifted `C_expected_sum` by 6.36%, over the 2% distortion threshold, so the boat failed the gate. `sigma_mu_E`, `sigma_mu_C`, `eps_mu_E`, and `eps_mu_C` are removed, and `mu_E` and `mu_C` now equal `mu_mu_E` and `mu_mu_C` directly, each with its existing `normal(value_normal_mu_mu_*, value_normal_sigma_mu_* = 2)` prior. Applied to `crab_bss_pooled.stan` and `crab_bss_pooled_weather_adjusted.stan`.
-   **This is not inference-preserving, unlike B1.3, B1.5, and B1.6.** It removes the extra HalfCauchy variance layer the hierarchy added on top of the `normal(., 2)` grand-mean prior. For a single section that layer has no interpretation (there is no between-section variation to partially pool), so it was spurious heavy-tailed prior mass; removing it tightens the prior tails on `mu_E` and `mu_C` while leaving a still-wide prior (SD 2 on the log scale spans roughly a 55-fold range at +/- 2 SD). The effect is negligible where the data is strong (both shore components, where the likelihood dominates) and mildly regularizing where it is weak (the boat). Expected effect on the next run: the boat divergences fall further and the catch distortion drops below the 2% gate, moving the boat from PE to BSS, though that is not guaranteed until the fit is run.
-   **Scope restriction.** The collapse hard-codes the single-cell assumption. It must not be ported to `crab_bss_gear_resolved.stan` or any multi-section or multi-gear configuration (`G > 1` or `S > 1`), where `sigma_mu` represents real between-cell variation and the hierarchy is required. The data-block inputs `value_cauchyDF_sigma_mu_E` and `value_cauchyDF_sigma_mu_C` are retained but unused, for R-interface compatibility, following the same convention as `n_effort_obs` in B1.5.
-   **PPC calibration hardened.** `bss_ppc_calibration` in `R_functions/model_diagnostics.R` was throwing on every fit in the v6.8 run, so `ppc_calibration_*.csv` and `ppc_pit_*.png` were never written (the structural and divergence diagnostics, which are separately wrapped, succeeded). The extraction of `lambda_E_S` / `lambda_C_S` is now robust to dropped size-1 array dimensions, and each observation is scored only on its finite predictive draws, so an extreme `lambda` draw (an `exp()` overflow in a weakly-identified fit, which sends an NA into `quantile()`) can no longer abort the whole PPC. The summary gains an `n_obs` column alongside the usable-draw count `n`.
-   Files changed: `crab_bss_pooled.stan`, `crab_bss_pooled_weather_adjusted.stan` (collapse, header); `R_functions/model_diagnostics.R` (hardened PPC); `BSS-GH-pooled-CPUE-model.Rmd` (header note only, no functional change); this documentation (Sections 9, 10, this entry). The covariate module inherits the augmented-model collapse and additionally gained a per-component `tryCatch` (v0.2.3).
