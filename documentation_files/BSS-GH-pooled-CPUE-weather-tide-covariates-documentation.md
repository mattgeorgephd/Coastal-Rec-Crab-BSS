...
| Version | Date       | Notes |
|---------|------------|-------|
| 0.2.3   | 2026-06-20 | Inherits pooled-model v6.9 (B1.7): the single-cell hierarchical scale (`sigma_mu_E` / `eps_mu_E` and the CPUE counterparts) is collapsed in the shared augmented model `crab_bss_pooled_weather_adjusted.stan`, so `mu_E` = `mu_mu_E` directly. With `G = 1`, `S = 1` that scale was an unidentified single-cell funnel and, after the B1.6 `sigma_IE` fix, the boat's last divergence source; the module's baseline and covariate fits inherit the de-funneled model. Not inference-preserving (it removes the extra HalfCauchy variance layer, meaningless for one section). Separately, each component's fit and LOO are now wrapped in a `tryCatch`, so a component that fails (a divergent augmented fit, a LOO or k-fold error, an out-of-memory) logs and the module continues to the remaining components, the convergence report, and the diagnostics, instead of aborting the run, the failure mode seen in the 2026-06-19 and 2026-06-20 runs, which stopped after `shore_ring_net_only`. See the pooled documentation v6.9 entry. |
| 0.2.2   | 2026-06-19 | Brought the BSS engine to pooled-model v6.8 parity: B1.5 marginalization of the effort overdispersion to a negative binomial and the B1.6 unconditional `sigma_IE` prior are both inherited from the shared augmented model `crab_bss_pooled_weather_adjusted.stan`. Added a fixed RNG seed (`bss_seed`, default 20260619) to all four Stan calls (the two cross-validation refits and the baseline/covariate main fits), so the module's fits and its k-fold CV are reproducible; per-fit model diagnostics (structural parameters, divergence localization, posterior predictive calibration) for every baseline and covariate fit via `R_functions/model_diagnostics.R`; and `session_info.txt` capture. The module's LOO-reliability convergence check is unchanged: the pooled model's v6.8 distortion-aware gate is deliberately not ported, because that gate tests whether divergences bias the summed totals, whereas the module's gate decides whether a LOO comparison is trustworthy, and LOO is more sensitive to divergences than the totals. See the pooled documentation v6.7 and v6.8 entries. |
| 0.2.1   | 2026-06-17 | Non-centered the AR initial states `omega_E_0` / `omega_C_0` in the augmented model `crab_bss_pooled_weather_adjusted.stan`, matching the pooled-track v6.6 fix (B1.3). The centered prior on these states is a funnel that the v6.5 pooled run showed drives near-total boat divergence; the module loads this augmented model for both its baseline and covariate fits, so it needs the same reparameterization for its BSS comparison to converge. The change is inference-preserving (same implied prior, scaled in transformed parameters). No module Rmd change. See the pooled documentation v6.6 entry for the full diagnosis. |
