...
#
# ---------------------------------------------------------------------------
# HOW TO READ THIS FILE. It is one flat list, in five sections, ordered so that a
# reader who wants to know WHAT THE MODEL DOES can stop at the end of section 2.
#
#   1. THE RUN                 the season window, the input workbooks, the sites, the
#                              sampler. Changes when the season or the machine changes;
#                              nothing here is a methodological choice.
#   2. THE METHOD OF RECORD    the adopted decisions that ARE Method v2.0. Each has a
#                              run behind it. Changing one changes what the model is.
#   3. DIAGNOSTICS             on in production, reported beside the estimate, and none
#                              of them changes a number.
#   4. SENSITIVITY / EXPERIMENT LEVERS   rarely touched. Experiment overrides, features
#                              that are built and deliberately off, rejected alternatives
#                              kept so they can be re-priced.
#   5. GEAR-RESOLVED ONLY      read by the cross-check model; the pooled model ignores it.
#
# A key a given model does not read is silently ignored by that model, which is why
# model-specific toggles can sit in one shared list.
# ---------------------------------------------------------------------------
#
# ---------------------------------------------------------------------------
...
#   the calendar     pot_closure_start, pot_closure_end, pot_open_date,
#                    census_start_date, census_end_date, commercial_opener
#                    (plus crabbing_holidays.xlsx rows and, for the spillover
#                    diagnostic only, fishery_opener_dates.xlsx rows)
#
# validate_season_window() warns when any of the structural dates falls outside the
# estimation window, which is the check that a rollback from a multi-season span needs.
# Two more groups to revisit on new data, both in section 2:
#
...
# per-stratum (year-month) logit random walk observed by the coastal crab samplers'
# interview contacts, with a combo-trip share `c` on its own walk; the within-day
# turnover is estimated from the OSP/trailer overlap and shared across the boat fits.
# Shore effort is the dock gear count expanded by a turnover derived from the I/E time
# column. Both components are on gear-deployments. Shore catch is zero-inflated negative
...
# in section 4.2, and it ships OFF so a production run is reproducible; turn it on
# deliberately.
# ---------------------------------------------------------------------------
#
...
# Everything a user changes season to season or to steer the model lives here, in the
# five sections mapped at the top of this file.

run_config <- list(

  # ============================================================================
  # 1. THE RUN: what season, from what files, with what sampler
  # ----------------------------------------------------------------------------
  # Everything in this section changes when you change the season or the machine you
  # run on. Nothing in it is a methodological choice. The nine per-season keys in 1.2
  # MUST be revisited together; validate_season_window() stops or warns if they do not
  # agree with each other or with the data.
  # ============================================================================

  # --- 1.1 Run identity and output labelling -------------------------------
  # run_tag names THIS run's output subfolder, so same-day runs of a model do not
  # overwrite each other: 05_output/<date>/<model>-<run_tag>. Blank = the driver appends an
  ...
  # every rung, so a clone running run_estimation.R did NOT reproduce the authoritative run,
  # and what it did produce was short by the 2023-24 commercial/charter catch: no vessel
  # tally or charter roster was ever kept for that season, so its census component is 0 and
  # estimate_comm_charter() only WARNS (D8). The span is not lost; it is written out with
  # its data prerequisites in 07_documentation/NEW_SEASON_GUIDE.md section 7.
  #
  # ALL NINE KEYS BELOW BELONG TO ONE SEASON. Change one, change them all, and let
  # validate_season_window() check you: it stops on a season_filter that does not match the
  # workbooks, and WARNS when a structural date falls outside the estimation window. The
  # 2026-09-11 ladder pin missed pot_open_date and commercial_opener in exactly that way.
  est_date_start    = "2024-09-16",   # first day of the estimation window
  est_date_end      = "2025-09-15",   # last day
  # A vector runs a multi-season span (e.g. c("2024-25", "2025-26")) with a matching est
  # window; every listed season needs rows in the workbooks and a holiday calendar. A stale
  # value stops the run loudly (validate_season_window.R).
  season_filter     = "2024-25",
  # MULTI-SEASON CLOSURE CALENDAR (2026-09-10, A14). NULL on a single season, which takes
  # the byte-identical scalar path below. Non-NULL outranks the scalar pair: one entry per
  # pot closure in the span, each with its season label; build_subseasons() then emits
  # ring_net_only_<season> / all_gear_<season> per closure, plus all_gear_pre_<season1> for
  # any window before the first closure, and the report rolls fits up to season totals
  # (season_totals.csv plus an HTML table).
  pot_closures      = NULL,
  # Pot-closure window: the period when pots are NOT legal (only non-pot gear: ring nets,
  # snares, traps). Given explicitly rather than assumed to start at the season start, so a
  # season whose start does not coincide with the closure start is supported. If the closure
  # starts after est_date_start or ends before est_date_end the driver adds the corresponding
  # all-gear period(s) automatically (03_R_functions/build_subseasons.R).
  # Keep pot_open_date = pot_closure_end + 1.
  pot_closure_start = "2024-09-16",   # = the season start
  pot_closure_end   = "2024-11-30",   # = the day before pots become legal
  # pot_open_date has exactly THREE consumers, all per-season and none of them a model term:
  # the "Pots open" vertical line on the season plots (pot_closure_markers), the gear
  # driver's monthly-panel vline, and build_subseasons()'s fallback for a NULL
  # pot_closure_end. A stale value is therefore COSMETIC, not silently wrong -- but it is
  # the key a rollback forgets, which is why validate_season_window() warns on it.
  #   2026-09-12 CORRECTION (CHANGE_REGISTER A14): the note that stood here said this date
  #   fed the L_effective I/E regression split, and called that a known multi-season
  #   approximation. IT DID NOT. estimate_L_effective() took pot_open_date as a formal
  #   argument and never referenced it in the body; its only predictors are yday (quadratic)
  #   and day type. The dead argument is gone. THE REAL multi-season approximation is that
  #   the regression POOLS every season's I/E days into one yday -> L curve, which is
  #   documented at the function; check the residuals by season in L_effective_ie_detail.csv
  #   before trusting a span whose seasons have different day-length behaviour.
  pot_open_date     = "2024-12-01",   # pots legal from this date
  # PER-SEASON CENSUS WINDOWS (2026-09-10). NULL on a single season. Non-NULL outranks the
  # scalar pair below: a named list season -> c(start, end), one commercial/charter window
  # per season, expanded recursively and summed with by_season kept.
  census_windows    = NULL,
  # THE COMMERCIAL/CHARTER CENSUS WINDOW: pots legal (Dec 1) through the last day before the
  # coastal commercial fishery opened and those vessels stopped fishing as recreational.
  ...
  #   2024-25  the tally ran Dec 3 to Feb 8, 2025; the coastal opener was Feb 11 (shipped).
  ...
  # The coastal commercial opener: reported, and the reason the census window ends where it
  # does. CORRECTED 2026-09-12: this file shipped "2025-01-01", which is not the 2024-25
  ...
  # --- 1.4 Sites, areas and the location filters ---------------------------
  ...
  # ============================================================================
  # 2. THE METHOD OF RECORD (Method v2.0)
  # ----------------------------------------------------------------------------
  # These keys ARE the method. Every one of them is an adopted decision with a run
  # behind it, recorded in 07_documentation/development_notes/CHANGE_REGISTER.md and
  # described in 07_documentation/BSS-GH-pooled-CPUE-model-documentation.md. Changing
  # one does not tune the model, it changes what the model IS, and the reported total
  # moves. If you change one, say so in the register and re-run the ladder rung that
  # isolates it.
  # ============================================================================
  ...
  # --- 2.2 The effort unit and the two turnovers ---------------------------
  ...
  # workbook. On the 2024-25 run that is 40 days spanning 2023-08 to 2026-08, of which 6
  ...
  # one never was.
  #
  # It stays pooled by default because 6 days is a thin basis for the second-largest mover
  ...
  # --- 2.3 The OSP port boat-count stream (the second boat effort stream) ---
  ...
  # --- 2.4 The crabbing fraction f, from the coastal crab samplers' interviews ---
  ...
  #
  # ADOPTED 2026-09-07 together with the weekly shore AR, in ONE adoption render, and
  # gate-confirmed 2026-09-08. It is part of the method of record; the prose above is the
  # evidence that put it there, kept so the decision can be re-examined rather than retold.

  # --- 2.6 AR temporal resolution: the per-population caps and the data floors ---
  ...
  # likelihood problem, not a resolution problem; see estimate_catch_zi in section 2.5.
  #
  # ADOPTED 2026-09-07 and gate-confirmed 2026-09-08: the shipped cap above is "weekly".
  # The gate was that the adoption render reproduce 05_output/20260904/pooled-CPUE-LZ-C1-
  # weekly-zi bit-identically, because C1 reached weekly through ar_force (an experiment
  # lever that bypasses the cap) and production reaches it through the cap: same
  # resolution, different routing, nothing else moving. It did, across 10,253 shared
  # parameter rows at full precision.
  ...
  # only the rung the ladder keeps reaches write_bss_diagnostics(); ar_rung_adequacy
  # (section 3.2) fixed that, and the port totals quoted here (71,450 -> 72,122) are
  # themselves superseded; see the box at the top of PIPELINE_STATUS.md.
  ...
  # --- 2.8 The commercial census and the charter expansion -----------------
  ...
  # ----------------------------------------------------------------------------
  # All ON in production and none of them changes a number. Each writes a CSV or a
  # report section; each is tryCatch-wrapped in the drivers so a failing diagnostic
  # cannot abort a multi-hour fit. Turn one off only to save time on a scratch run.
  # ============================================================================

  # --- 3.1 Estimator-side diagnostics --------------------------------------
  ...
  # --- 3.3 The other-fishery spillover diagnostic (pooled report only) -----
  ...
  # ============================================================================
  # 4. LEVERS FOR SENSITIVITY AND EXPERIMENTS -- RARELY TOUCHED
  # ----------------------------------------------------------------------------
  # Nothing here is part of the method of record. Each is either an experiment lever
  # that bypasses a production mechanism, a feature that is built and deliberately off,
  # or a rejected alternative kept so it can be re-priced. THREE OF THESE FORCE A STAN
  # RECOMPILE when changed: razor_dig_mode, estimate_cpue_density, estimate_catch_zi.
  # If you are reading run_config.R to find out what the model does, you can stop here.
  # ============================================================================

  # --- 4.1 Per-fit overrides that bypass production routing ----------------
  ...
  # --- 4.2 The AR escalation ladder (opt-in; costs one extra fit per failed rung) ---
  ...
  # --- 4.3 Other-fishery opener effort covariates (BUILT, INERT) -----------
  ...
  # tests in the 2024-25 diagnostic came back null (p = 0.29 to 0.90). The screen above is
  # an EFFORT screen only.

  # --- 4.4 The razor-clam dig covariate (BUILT, INERT; forces a Stan recompile) ---
