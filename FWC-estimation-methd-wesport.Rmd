---
title: "FWC-estimation-method-westport"
author: "Matt George"
date: "2026-03-06"
...
## Load Packages
load.lib<-c("tidyverse", "lubridate", "suncalc", "gt", "patchwork", "rstan", "here","knitr")
...
prep_inputs_bss_crab <- function(days, dwg_summ, est_catch_group, period, params) {
  ...
  # Subsample interviews for faster fitting (comment out for full run)
  if (nrow(int_d) > 1000) {
    set.seed(42)
    int_d <- int_d |> slice_sample(n = 1000)
    intA_d <- intA_d |> filter(interview_id %in% int_d$interview_id)
  }
...
# # run this to evaluate whether your machine is set up for parallel processing
# library(parallel)
# cl <- makeCluster(4)
