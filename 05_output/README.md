...
| Commercial/charter census (2026-09-08+) | `census_daily.csv`, `census_variance.csv` | Every calendar day of the census window flagged observed (tally day, exact) or, under the shipped `census_expansion = "none"` (2026-09-09), no-operation (zero); under `"day_type"` the unsampled days carry the day-type mean. 2026-09-10: under `charter_frame = "roster"` the table also carries `roster_trips` (the charter roster's trips on a tally day), `roster_only_trips` (its trips on a day without a tally, source "charter roster (no tally)") and `charter_n` (the charter count used, the larger of the two frames). `census_variance.csv` holds the per-stratum variance behind the census SE (`census_uncertainty` decides whether it enters the port interval). |
