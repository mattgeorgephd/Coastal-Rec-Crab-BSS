...
| PE vs BSS | `pe_port_summary.csv`, `pe_vs_bss_comparison.csv`, `monthly_pe_vs_bss.csv` | Point Estimator results and the side-by-side reconciliation used by the convergence gate. 2026-09-08+: the census row's `method_selected` states the observed / imputed split and the imputation SE. **Read `pe_empty_effort_strata.csv` alongside these**: the PE point depends on how unsampled cells are filled, and `bss_reported = FALSE` on a component means its PE point IS the reported estimate, entering the port total as a constant. |
| PE unsampled cells | `pe_empty_effort_strata.csv` | Per component, how much of the PE rests on thin cells and what the three levers did to it (rewritten 2026-09-12). Columns: `effort_fill` / `cpue_fill` / `variance` (the levers as resolved for this run); `n_empty_strata` and `n_empty_days` (cells with calendar days and NO sampled day); `n_single_strata` and `n_single_days` (**new**: cells resting on exactly ONE sampled day, which had no variance at all before this date and are the larger half of D21); `thin_day_fraction` (the two together, over calendar days: 0.44 for shore all-gear and 0.43 for boat all-gear on 2024-25); `imputed_effort` and `imputed_effort_share`; `zeroed_effort_bias` (**new**: the effort an unsampled cell WOULD carry, so `pe_empty_effort_stratum = "zero"` states its own cost -- an omitted cell's error is a BIAS and no SE represents it); `effort_se` against `effort_se_sampled_only` (the honest interval and the pre-2026-09-12 arithmetic, side by side, so no earlier number becomes unreachable) and `effort_cv`; `exceeds_5pct_at_zero`. |
| Commercial/charter census (2026-09-08+) | `census_daily.csv`, `census_variance.csv` | Every calendar day of the census window flagged observed (tally day, exact) or, under the shipped `census_expansion = "none"` (2026-09-09), no-operation (zero); under `"day_type"` the unsampled days carry the day-type mean. 2026-09-11: the table carries the two components separately, `est_dung_char` (the charter expansion's crab for that day's frame trips) and `est_dung` (the day's total), plus `charter_trips` (the frame's trips that day) and the tally columns; a day with charter trips but no tally reads "charter roster (no tally)". It sums to the component in every frame x expansion combination, which the harness asserts. `census_variance.csv` holds the per-stratum variance behind the census SE (`census_uncertainty` decides whether it enters the port interval). |
| Sampler shifts (2026-09-09+) | `shift_coverage_daily.csv`, `shift_coverage_summary.csv`, `shift_coverage_return_profile.csv`, `contact_hour_by_trip_type.csv`, `contact_hour_bins_by_trip_type.csv`, `crabbing_share_by_contact_hour.csv` | Per shift day the union of the sampler windows, the hours in port and the share of the day's boat returns inside them (from the boat I/E return profile); the contact hour by trip type and the crabbing share of typed contacts by hour bin. The evidence on the shift-time caveat behind the crabbing fraction. |
...
| OSP and crabbing fraction | `osp_trailer_overlap_calibration.csv`, `osp_trailer_overlap_pairs.csv`, `osp_trailer_overlap.png`, `osp_coverage_audit.csv` | OSP-vs-trailer calibration, the paired overlap days, the calibration plot, and the OSP coverage audit (written once per run, boat only). The posterior `kappa_OSP_out` and `f_crab_out[...]` appear inside `bss_full_summary_private_boat_all_gear_*.csv`. 2026-09-08+: `crab_fraction_strata_<fit>.csv` (boat fits) labels each `f_crab_out[k]` with its stratum (YYYY-MM), the walk link, the contact and OSP counts that informed it, and the posterior median / 95% interval; 2026-09-09+ also the typed contact counts, the combo share `c` per stratum and the implied crab-only share `f(1 - c)`. `bss_summary_*` carries `sigma_f_out`, `cfi_kappa_out` and, with typed contacts or an OSP crabbing-only column, `combo_c_out`, `sigma_c_out`, `cfc_kappa_out`. |
...
| Model adequacy (2026-08-27+) | `model_adequacy.csv` | Reported BESIDE the convergence gate, never gating: `p_loo` as a fraction of `n_obs` (worst stream), the Pareto k > 0.7 count, worst randomized-PIT coverage deviation, and `flag_miscalibrated`. The statistic set exists because six configurations once passed every gate criterion while spanning 44% on the boat. |
| AR ladder (2026-08-25+) | `ar_escalation_log.csv` | One row per fit attempt when `ar_escalate` is on (one per fit otherwise): resolution, `P_n`, gate verdict, each rung's own estimate and interval, per-rung adequacy (2026-09-06+), and a `selected` flag marking the reported rung. |
| Structural-parameter provenance (2026-08-27+) | `structural_params_*` (`decoupled` + `decoupled_reason` + blanked `estimate` columns), `decoupled_audit.csv` (retro-fitted folders only) | Several parameters carry a proper prior unconditionally, so an unused one reports its PRIOR in the same columns as an estimate; the flag says which rows are priors. |
| Stan console + timings (2026-08-26+ / 2026-09-05+) | `stan_console_*_att<k>.log`, `run_timings.csv` | The per-fit Stan console (kept because `results='hide'` chunks are otherwise undiagnosable) and per-section wall-clock, one row per fit attempt. |
| PPC draws (2026-09-04+, NOT committed) | `ppc_draws_*.rds` | The draw objects the R-side predictive statistics are computed from, persisted so a diagnostic-code fix is a recomputation rather than a re-fit. `*.rds` is git-ignored, so these exist only on the machine that ran the fit; no recompute path reads them back yet (CHANGE_REGISTER D5). |
| Run metadata | `run_parameters.txt`, `session_info.txt` | The exact parameters (untruncated since 2026-09-04; earlier dumps are capped at 99 keys and unreliable for config comparison) and the R/package session for that run. |
...
**Cross-run files at the `05_output/` root.** Not everything lives inside a dated run folder. `osp_validation_summary.csv` collects one row per OSP validation fit across both models (`run_osp_validation.R`, merged by key since 2026-09-01 so RESUME cannot duplicate rows). Each dated batch runner also writes its pass/fail verdicts here as `<batch>_verdicts.csv` (`osp_validation_verdicts.csv`, `shore_ar_zi_2026-09-03_verdicts.csv`, `ladder_zinb_2026-09-04_verdicts.csv`, `adoption_2026-09-07_verdicts.csv`, ...): stage, criterion, observed value, threshold, verdict, and the reasoning, written via a keyed merge so a re-run updates rows rather than appending or truncating. Corrected verdicts carry supersession columns beside the originals rather than being rewritten.
