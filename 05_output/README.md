...
| Commercial/charter census (2026-09-08+) | `census_daily.csv`, `census_variance.csv` | Every calendar day of the census window flagged observed (tally day, exact) or imputed (day-type mean), with the per-day estimate; and the per-stratum imputation variance behind the census SE (`census_uncertainty` in `run_config.R` decides whether it enters the port interval). |
