...
| Commercial/charter census (2026-09-08+) | `census_daily.csv`, `census_variance.csv` | Every calendar day of the census window flagged observed (tally day, exact) or, under the shipped `census_expansion = "none"` (2026-09-09), no-operation (zero); under `"day_type"` the unsampled days carry the day-type mean. `census_variance.csv` holds the per-stratum variance behind the census SE (`census_uncertainty` decides whether it enters the port interval). |
