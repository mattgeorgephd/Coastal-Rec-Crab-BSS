...
| `test_improvements_2026-08-25.R` | Dependency-light regression harness (no rstan, seconds to run) for the functions the 2026-08-25 batch added or changed, plus a guard on the shipped defaults in `run_config.R`, on the sampler-override whitelist, on the retro audit tables, and on the Stage 5 runner's hard-coded baselines. **199 assertions.** Run it before committing to any long fit. |
