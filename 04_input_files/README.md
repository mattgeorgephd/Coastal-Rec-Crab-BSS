...
| `fishery_opener_dates.csv` | pooled driver only, via `prep_fishery_events` | `read.csv(here("04_input_files","fishery_opener_dates.csv"))` | Consolidated daily MA2 finfish (salmon / halibut / bottomfish) and coastal razor-clam-dig opener calendar, one OPEN/CLOSED row per day. The **primary** source for the fishery-spillover diagnostic. |
| `MA2-fishing-dates-2023-2026.xlsx` | pooled driver only, via `prep_fishery_events` (fallback) | `read_excel(here("04_input_files","MA2-fishing-dates-2023-2026.xlsx"), sheet = "data")` | Source workbook for the MA2 finfish openers. Read only as a **fallback** when `fishery_opener_dates.csv` is absent. |
| `razor-clam-dig-dates-2021-2025.xlsx` | pooled driver only, via `prep_fishery_events` (fallback) | `read_excel(here("04_input_files","razor-clam-dig-dates-2021-2025.xlsx"), sheet = "data")` | Source workbook for the coastal razor-clam-dig dates. Read only as a **fallback** when `fishery_opener_dates.csv` is absent. |

The pooled driver names the I/E workbook through parameters (`ie_data_file = "ingress_egress.xlsx"`, `ie_sheet = "data"`), so a different I/E file or sheet can be swapped in without touching code. The three core CSVs (effort, interview, tally) are referenced by literal name in both drivers. The fourth CSV (`fishery_opener_dates.csv`) and its two `.xlsx` fallback workbooks are read by the pooled driver only, through `prep_fishery_events`, and each can be renamed through a `params` key.
