...
| Version | Date       | Notes |
|---------|------------|-------|
| 0.2.3   | 2026-06-21 | Each component's fit and LOO are wrapped in a `tryCatch`, so a component that fails (a divergent augmented fit, a LOO or k-fold error, an out-of-memory) logs and the module continues to the remaining components, the convergence report, and the diagnostics, instead of aborting the run, the failure mode seen in the 2026-06-19 and 2026-06-20 runs which stopped after `shore_ring_net_only`. NOTE: this version initially also inherited the pooled-model B1.7 single-cell collapse in the shared augmented model `crab_bss_pooled_weather_adjusted.stan`, but B1.7 was reverted after it made the pooled shore all-gear fit hang (max_treedepth saturation, the single-scalar level reconciling against the 289-state daily AR); the augmented model is restored to the v6.8 hierarchy. The `tryCatch` is retained. See the pooled documentation v6.9 entry. |
