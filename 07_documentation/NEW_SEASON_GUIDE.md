...
|---|---|
| `effort_combined.xlsx` | Add the season's rows (`season` label must match what you will put in `season_filter`, exactly). |
| `interview_combined.xlsx` | **Do not hand-edit (2026-09-09).** Drop the new creel-database export into `04_input_files/raw/` and run `Rscript 04_input_files/build_interview_combined.R raw/<export>.xlsx`; the builder keeps the 17 legacy columns, adds `trip_type` / `trip_type_class` / `creel_area` / `interview_time`, fills `gear_tampered`, and prints a column-by-column comparison with the previous workbook. The `Trip Type` labels must classify (crab only / `<fishery> & Crab` / finfish only / non fishing); an unrecognised label is flagged. |
| `sampler_shifts.xlsx` | Drop the survey export into `raw/` and run `Rscript 04_input_files/build_sampler_shifts.R`; flagged rows (a check-out typed on a 12-hour clock, a missing check-out) are listed and held out of the hours. |
