...
**Update 2026-07-10 (after the gear-resolved 7/10 run, commit `d1968f3`):** the gear-resolved section below now carries a per-item **Status after the 2026-07-10 run** block (GR-7..GR-15 verdicts plus new items GR-16/GR-17). That run also re-opened the POOL-1/POOL-3 decision: it validated the boat deployment scale and showed the pooled boat is ~30% too high, so POOL-1 + POOL-3 were reassessed from held to recommended and then **IMPLEMENTED in v7.6** (see the pooled table). The pooled boat now moves; confirm with a run.
...
**POOL-1 + POOL-3 — IMPLEMENTED in v7.6 (2026-07-10).** Held 2026-07-10, reassessed as recommended after the gear-resolved 7/10 run, then implemented. One entangled fix that moves the publication boat number (pooled boat BSS all-gear ~56,266 in the 20260708 run) onto the gear-deployment scale. Evidence, the spec, and the as-built note follow:
...
- *Implemented in v7.6 (2026-07-10).* Done as specified, mirroring `crab_bss_gear_resolved.stan`. `crab_bss_pooled.stan`: R_T -> R_G_boat with `R_G_boat ~ lognormal(log(4), 0.5)` (unconditional, F1), `T_I ~ NB2(lambda_E / R_G_boat, r_E)`, `Gear_A_boat ~ poisson(R_G_boat)`, plus `effort_scale_gear` / `E_scale` so `E = lambda_E * E_scale * L` carries h's unit. Driver: adopts the shared `bss_effort_spec.R` for the boat (h = number_of_gear, L = tau_boat, estimate_L = 1); `run_pe` and the monthly PE share use the same deployment scale; expansion reporting extracts R_G_boat; the old `boat_use_gear_hours` flag and the R_T / T_A_int / A_A_trailer data are removed. Shore is unchanged by POOL-1/POOL-3 (E_scale = 1). Validated by R parse checks and an adversarial review against the gear-resolved reference (all checks pass; the Stan likelihood/GQ lines are byte-identical to the reference), but NOT yet confirmed by a pooled run. Next: run pooled v7.6, confirm the boat gates pass and the boat number lands near the gear-resolved ~43k (down from ~56k), and if the single-cell funnel appears set `collapse_mu_hier = list(private_boat = TRUE)` and/or move the boat AR to monthly (as gear-resolved does).
...
1. **Run pooled v7.6** — POOL-1 + POOL-3 are now implemented (boat on the `R_G_boat` / `tau` deployment scale). Run pooled v7.6 to confirm the boat gates pass and the boat number lands near the gear-resolved ~43k (down from ~56k), and to refresh all totals (POOL-2 also raises shore). If the boat single-cell funnel appears, set `collapse_mu_hier = list(private_boat = TRUE)` and/or move the boat AR to monthly. NOTE: the driver `.Rmd` was NOT in the v7.5 commit on main, so integrate the full v7.6 driver (it carries POOL-2/4/5/6 and POOL-1/3) or the Stan will error on the missing `collapse_mu_hier`.
