...
**Repo:** `Coastal-Rec-Crab-BSS`, `main`. **Method of record:** Method v1.0 (frozen against pooled code v7.4); the code has advanced past v7.9 with the Tier-2 batch (nine items, 2026-07-13), and the authoritative run is now **Run 1** (`20260713/pooled-CPUE`). The 2024-25 reference numbers in the method documents remain pre-refresh; they should be regenerated to the Run 1 totals (port total 83,035; Section 1).
...
| Gear-resolved model (`crab_bss_gear_resolved.stan`, `BSS-GH-gear-type-CPUE-model.Rmd`) | framework **v5.6** (2026-07-12) | Production, validated by Run 2 (`20260713/gear-type-CPUE-model`, 3/3 fits pass). Shore + boat on gear-deployments; `G = 1` (per-gear CPUE inert, gear split from PE apportionment, see GR-7). |
...
- **`05_output/`**, dated run folders. Committing full run outputs is a known hygiene problem (T4.2/ORCH-26) and keeps growing with each run. The current authoritative run is `20260713/pooled-CPUE` (Run 1).
