...
1. *The trailer stream is over-covered in every configuration.* `coverage_50` is 0.667 to 0.692 at monthly against a nominal 0.500, which is 4.6 to 5.3 sampling SDs high (n = 195). The shared turnover improves it slightly; nothing fixes it. The daily AR makes it far worse, which is a different problem from the one it was over-covered by. This is a real open item and it is new information from this batch.
