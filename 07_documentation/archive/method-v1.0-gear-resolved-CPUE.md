> # ARCHIVED. THIS IS METHOD v1.0, AND IT IS NOT THE METHOD OF RECORD.
>
> Method v1.0 was frozen against pooled code v7.4. The method of record is now
> **Method v2.0**, `07_documentation/BSS-GH-gear-type-CPUE-model-documentation.md`, which
> describes the model as it runs today. This file is kept unaltered below the banner as the
> provenance record of what v1.0 said, because several later decisions are only legible as
> departures from it.
>
> **Every number in this file is historical.** The authoritative run and its total live in
> one place, the box at the top of
> `07_documentation/development_notes/PIPELINE_STATUS.md`.
>
> **Where v1.0 and v2.0 differ in the MODEL, not merely in settings** (each adopted with a
> run behind it; see `development_notes/CHANGE_REGISTER.md`):
>
> | | Method v1.0 | Method v2.0 |
> |---|---|---|
> | boat effort streams | trailer counts | trailer counts **+ the OSP daily port count of returning vessels** |
> | crabbing fraction `f` | a scalar, 0.30, on a Beta(6,14) prior | a **per-stratum (year-month) logit random walk** observed by the coastal crab samplers' interview contacts, with a combo-trip share `c` on its own walk |
> | boat turnover | `tau_boat` fixed at 1.2 from two I/E days | resolved per window from the **OSP/trailer overlap calibration**, and shared across the boat fits as one estimated `tau_bar` |
> | shore turnover | `tau_shore` fixed at 1.7 (arrivals / peak presence) | **derived** from the I/E `time` column at the creel count hours |
> | shore catch likelihood | NB2 | **zero-inflated NB2** |
> | shore all-gear AR | daily | **weekly** |
> | commercial + charter | one day-type-stratified census expansion | an **exact commercial census** over the tally days **plus a charter expansion** over the charter trip roster, stratified by vessel, with a finite-population correction |
> | PE unsampled cells | priced at zero effort | filled from a **month-local day-type donor**, with an impute-aware variance |
> | window | one season | one season, part of a season, or a **multi-season span** |
>
> Archived 2026-09-12.
...
|---|---|---|---|---|
| Pot closure | Sep 16 to Nov 30 | Ring nets, snares, foldable traps | 3 (Ring Net, Trap, Snare) | Biweekly |
| All-gear | Dec 1 to Sep 15 | All gear including crab pots | 4 (Pot, Ring Net, Trap, Snare) | Monthly |

The "pot closure" sub-season was formerly labeled "ring-net only", a misnomer since non-pot gear other than ring nets is also legal in the closure; the internal key stays `ring_net_only` for output-filename continuity and the reports display "Pot closure". The closure window is set explicitly in `run_config.R` via `pot_closure_start` / `pot_closure_end` (added 2026-07-13) rather than assumed to run from the season start to `pot_open_date - 1`; the shared builder `03_R_functions/build_subseasons.R` derives the sub-seasons and adds pre/post all-gear periods automatically if the closure falls mid-season. A multi-season span lists one closure per season in `pot_closures` (added 2026-09-10), which this driver consumes through the same shared builder; the season-totals report table is pooled-driver-only for now. Season plots mark the closure start and the pots-open date with vertical lines.

The number of gear types set up for modeling in a sub-season is determined by two filters applied in sequence.

**Regulatory exclusion.** Each sub-season carries a `gear_exclude` list of gear prohibited by regulation during that period. For the pot-closure sub-season, pots are excluded. This is enforced structurally: regardless of interview contents, the model will not set up a CPUE process for a prohibited gear type. Any interview that mentions an excluded gear type (typically a multi-gear crabber naming gear they own rather than what they deployed) has its fractional weight redistributed to the remaining reported types. An interview recorded as "Pot, Ring Net" during the pot-closure sub-season contributes 100% of its weight to Ring Net rather than 50% to each. This matters because even a handful of phantom gear mentions (5 to 6 fractional interviews in a 76-day sub-season) can, with near-zero data to constrain a CPUE process, drive hundreds of divergent transitions and overflow in the generated catch. Encoding the known fishery structure prevents that.
...
The pot-closure sub-season uses biweekly BSS periods rather than monthly: with only about 76 days and 2.5 calendar months, monthly periods give too few AR(1) transitions for effective temporal smoothing, while biweekly periods yield about 5 to 6 periods.
...
`X_open` / `B_open` are the other-fishery opener effort covariates (2026-08-25); `K_open = 0` is the production default and removes the term. Because the gear-resolved effort mean is a single shared level split across gears by the `O` offset, the opener covariates attach to that shared level rather than per gear. Selection, the multiplicity control, and the interaction with the crabbing fraction are identical to the pooled model; see the pooled document, Sections 15 and 16.

**Weekend definition (changed 2026-08-25).** `w[d]` marks Saturday and Sunday. Friday was previously included; within month it runs at 0.43x Saturday for the shore gear count (p = 2.4e-06) and 0.60x for boat trailers (p = 0.003) while sitting on the Monday-Thursday mean, so it is a weekday in this fishery. Note the gear-resolved model has no separate holiday CPUE term and folds holidays into `B1_C`, which is why its weekend CPUE numbers differ from the pooled model's.
...
**Escalation ladder (`ar_escalate`, added 2026-08-25, ships OFF).** Independently of `ar_adaptive`, the ladder starts each component at the finest rung and refits one rung coarser on each convergence-gate failure, stopping at the first rung that passes, so the fixed per-sub-season periods become a starting point rather than a verdict. Every rung is a real multi-hour fit and the gear-resolved latent AR is `P_n x (G*S)`, roughly four times the pooled dimension at four gear types, so the cost here is higher than in the pooled track; treat the ladder as an occasional audit rather than a routine setting. `ar_escalation_log.csv` records each attempt and the verdict that triggered the next. Full rationale in the pooled document, Section 14.1.

#### 14.2 Population effort construction and the gear-deployment effort unit

As of framework v5.5 **both fitted components run on gear-deployments**, routed through the shared module `03_R_functions/bss_effort_spec.R` so the BSS and PE always share a unit. The CPUE denominator is the number of gear deployments (`h = number_of_gear`), and the daily effort expansion is `L = tau`, the deployment turnover (trips per gear-slot per day: how many separate crabber trips use the same gear slot over a day, NOT how many times a single crabber checks their gear). Effort in deployment units is formed as `E = lambda_E * E_scale * L`, and the run asserts that `E` and `h` carry the same unit before sampling.
...
On I/E survey days, an observed ingress count enters as a lognormal constraint on the predicted trips (`lambda_E * L` on the shore/deployment scale, `lambda_E / R_G_boat * tau` on the boat group scale), a second independent constraint on the latent effort state that identifies the turnover `tau`. **Corrected 2026-08-25:** the SHORE observation is now the crabber **arrival** count under the gear-deployment unit, matching that predicted trip quantity. Between v5.5 and 2026-08-25 it was still crabber-hours, so the shore stream was comparing crabber-hours against a predicted crabber-trip count (roughly 331 observed against 80 predicted on a typical Float 20 day). `bss_effort_spec()` now owns the pairing for both populations; `ie_shore_obs_unit = "crabber_hours"` reproduces the old behaviour. This changes the shore posterior and must be confirmed by a run. See the pooled document, Section 14.5, for the full note and the `L_effective`-versus-turnover relationship. When no I/E data fall in a fit's window, the I/E likelihood contributes nothing and `tau` rests on its prior. The prior on the I/E scale `sigma_IE` is applied unconditionally (parity port B1.6), so that with no I/E data `sigma_IE` is proper rather than an improper flat direction; because it is decoupled from effort and catch in that case, this leaves those posteriors unchanged.
...
- **Boat catch rests on a thin `tau_boat` (GR-12), now closed by OSP.** `tau_boat = 1.2` was anchored on only two within-window boat I/E days (turnover 1.00 and 1.29), and boat catch is directly proportional to it. The OSP boat-count stream (below, production ON) supplies a dense 148-day series that identifies the boat turnover directly, so `tau_boat` no longer leans on its prior. Resolved 2026-07-31: the trailer count is an instantaneous snapshot, so the OSP-implied turnover (~2.7 to 3.0) is real and the old ~1.2 was a ~2x under-count; see the OSP subsection below.
...
**OSP boat counts, the crabbing fraction f, and OSP-informs-tau (branch `OSP-boat-count-incorporation`, 2026-07-31).** The gear-resolved model carries the same boat-effort additions as the pooled model.

- **OSP second effort stream.** The OSP Westport Boat Launch daily private-boat total (`04_input_files/WBL_boat_counts.xlsx`, `fetch_osp_boat_counts`, toggle `use_osp_boat_counts`, production ON) is a second boat effort observation on the same latent boat effort, scaled by `kappa_OSP`; in the gear-resolved likelihood the OSP mean sums the boat effort over gear (`sum(lambda_E_S[...])`). It contributes on its 148 operating days (mid-March to mid-October) and degrades to trailer-only outside them. Validation (Step 2) raised the boat about 10% and tightened boat-effort CV 11.9% to 11.0%.
- **Crabbing fraction f.** Because trailer and OSP counts are ALL boats, `use_crab_fraction` (production ON) multiplies boat effort and catch by f in the boat generated quantities only, decoupled from sampling (CPUE invariant; the boat scales exactly linearly in f). f is a Beta prior centered on `crab_fraction_set` (production 0.3, a placeholder pending the WBL egress pilot, updated by the `boats_crabbing` / `boats_total` classification in `ingress_egress.xlsx`). It is the single largest lever on the boat harvest.
- **OSP-informs-tau (`osp_scale_is_tau`, production ON).** Closes GR-12 by letting the OSP series identify `tau_boat` (the OSP mean uses `L` = `tau_boat` instead of the free `kappa_OSP`). Since the trailer count is an instantaneous snapshot, the OSP/trailer turnover (posterior `kappa_OSP` = 3.15, 95% [2.50, 3.91]) is real and the old `tau_boat` ~1.2 was a ~2x under-count; turning it on roughly doubles the boat (validation Step 6 boat ~27.5k vs Step 3b ~13.8k). Caveat: this assumes the snapshot is timed representatively (not a fixed daily peak).
- **Production configuration (adopted 2026-07-31):** `use_osp_boat_counts = TRUE`, `use_crab_fraction = TRUE` with `crab_fraction_set = 0.3`, `osp_scale_is_tau = TRUE`. Confirmed by the production run `05_output/20260805/gear-type-CPUE-model-boat-count-validation-run` (all fits converged): the private-boat harvest is **27,524** Dungeness kept (BSS median) and the **port total is 66,461** (95% CI 50,259 to 91,309; PE port 44,810). The run reproduces the pre-merge gear validation (Step 6) fit-for-fit, and the two production tracks reconcile to 0.6% on the boat and 1.3% on the port. Full evidence and the two open decisions (the pilot f_hat, and confirming the snapshot timing) are in `development_notes/osp-validation-review-2026-07-31.md`.
...
| gear-deployments | Count of gear a crabber had in the water on that trip; the CPUE denominator `h = number_of_gear` and the effort unit for both components. **Not** a "pot lift": repeat checks of the same gear slot are not extra deployments, and slot re-use across the day is carried by the turnover `tau` (corrected 2026-08-25) |
...
**2026-08-25 improvement batch (branch `OSP-boat-count-incorporation`; not yet run).** The gear-resolved track receives the same eight improvements and the same two defect fixes as the pooled track, so the two cannot drift: the shore I/E observation-unit correction (Section 14.8, the one that moves the shore number), the Saturday/Sunday weekend definition (Section 14.1), the interview floor at 15 with a post-filter second floor, opener effort covariates with a multiplicity-controlled screen, the AR escalation ladder, the OSP crab-only hard lower bound on the crabbing fraction f, and the four-arm incomplete-trip treatment diagnostic. Two residues specific to this track were also fixed: the daily-combined series applied the shore crabber-hours formula (crabbers x day length, catch per crabber-hour) to the boat as well, using the shore multiplier against a trailer count, and the by-gear incomplete-trip bias was measured only in crabber-hours, the denominator this pipeline's own linearity test rejects; it is now reported in gear-deployments with the crabber-hour figure retained for continuity. Rationale for each item lives in the pooled document to avoid two copies drifting; the batch is unrun, and the I/E unit fix should be isolated in its own run first.
...
- **OSP boat-count incorporation (branch `OSP-boat-count-incorporation`, 2026-07-31).** The boat gains the OSP Westport Boat Launch daily boat-total as a second effort stream (scaled by `kappa_OSP`; the gear-resolved OSP mean sums boat effort over gear), a crabbing fraction f applied in the boat generated quantities (production 0.3 placeholder, boat linear in f, CPUE invariant), and `osp_scale_is_tau` (production ON) which closes GR-12 by identifying `tau_boat` from the dense OSP series. With the trailer count confirmed an instantaneous snapshot, the OSP-implied turnover (~2.7 to 3.0) is real and `tau_boat` ~1.2 was a ~2x under-count, so production boat is 27,524 Dungeness kept, port total 66,461, confirmed by the run `05_output/20260805/gear-type-CPUE-model-boat-count-validation-run`. Validated by a 14-run batch plus the production confirmation run (both tracks reconcile within 1 to 2%); see `development_notes/osp-validation-review-2026-07-31.md`. Two interview data fixes ride along (non-crabbing `number_of_gear = 0` and `gear_tampered = 1` rows dropped).

The run-driven Stan fix markers (F1 the unconditional `R_G_boat` prior; F2 the gear-deployment scale, the single largest quantitative move in the model's history; F4 the CPUE diagnostics; F5 reporting; P0 the point-estimator population and ratio-of-sums fix; P1 the configurable effort unit; P2 the mu-hierarchy collapse) and the pooled parity ports (B1.3, B1.5, B1.6, B1.8, B1.9) are recorded, with their before/after numbers and the F1/F2 and P0 working notes, in **`BSS-GH-gear-type-CPUE-model-development-history.md`**. The outstanding backlog (GR-7 to GR-17) lives in `07_documentation/development_notes/PIPELINE_STATUS.md`.

**Reference-run figures (refreshed 2026-08-05).** These are the confirmed figures from the production run `05_output/20260805/gear-type-CPUE-model-boat-count-validation-run` (gear-deployment scale, production config OSP + f = 0.3 + `osp_scale_is_tau`), which reproduces the pre-merge validation fit-for-fit. They supersede the pre-v5.5, pre-OSP totals; the pre-OSP f = 1 boat figure is retained for provenance.

| Component | Reference figure (BSS median) | Basis and status |
|---|---|---|
| shore_ring_net | 6,258 | gear-deployment scale; confirmed 2026-08-05 |
| shore_all_gear | 20,124 | gear-deployment scale; confirmed 2026-08-05 |
| private_boat_all_gear | 27,524 (production); was about 43,314 at f = 1, pre-OSP | gear-deployment scale; production adds OSP (`osp_scale_is_tau`, closes GR-12) and f = 0.3; the 43,314 was the f = 1, `tau_boat` = 1.2 baseline |
| comm/charter (census) | about 11,986 | day-type-stratified vessel-tally census |
| **port total** | **66,461** (95% CI 50,259 to 91,309) | assembled BSS total; PE port 44,810 |
