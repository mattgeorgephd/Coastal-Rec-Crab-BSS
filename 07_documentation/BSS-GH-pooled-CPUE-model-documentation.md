...
**Current pipeline code:** v7.7 (2026-07-11). Since v7.4 the code has advanced three times, each in a way that moves published totals: v7.5 added the incomplete-trip filter (raises the shore estimate), v7.6 moved the private boat onto gear-deployments (moves the boat total, expected roughly -25%), and v7.7 moved the shore onto gear-deployments (moves the shore total). The method is unchanged by these; they are effort-unit and filtering corrections, not a re-derivation. But the reference-season (2024-25) numbers quoted throughout this document are therefore pre-refresh: they must be regenerated by a v7.7 run before they are cited as final. Where a number below is described as current, read it with this caveat.
...
Method v1.0 is the frozen method label; it was first frozen against pipeline code **v7.4**. The current pipeline code is **v7.7** (2026-07-11). The method itself is unchanged since v1.0, but three code revisions each move published totals and so make the 2024-25 reference numbers in this document pre-refresh (a v7.7 run is required before they are cited as final, as noted in the header): v7.5 added the incomplete-trip filter, v7.6 moved the boat onto gear-deployments, and v7.7 moved the shore onto gear-deployments. The model began as an adaptation of the WDFW freshwater-creel state-space framework and was hardened over a sequence of versions in response to a 2026-03-31 model critique and an extended convergence-debugging effort focused on the private boat. The arc in one screen:
